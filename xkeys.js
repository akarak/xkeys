...
// Note:
// This is based on info from http://xkeys.com/PISupport/DeveloperHIDDataReports.php
...
class XKeys extends EventEmitter {
	constructor(pid) {
		...
		if (productId) {
			this.device = new HID.HID(VENDOR_ID, pid);
		} else {
		...
		if (!this.deviceType) {
			throw new Error(
				"Unknown/Unsupported X-keys: '" + productId + "' (id: " + productId + ").\n" +
		...
		this.device.on("data", data => {
			...
			let buttonStates = {};
			let buttonStates2 = {}; // alternative buttons, such as the program switch "PS"
			let analogStates = {}; // Analogue states, such as jog-wheels, shuttle etc

			let d, bit;
			for (let x = 0; x < this.deviceType.columns; x++) {
				for (let y = 0; y < this.deviceType.rows; y++) {
			...
			for (let buttonStateKey in buttonStates) {
